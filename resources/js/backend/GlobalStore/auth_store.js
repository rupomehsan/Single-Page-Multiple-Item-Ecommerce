...
export const auth_store = defineStore("auth_store", {
  ...
  getters: {
    ...
    has_permission: (state) => (permission_slug) => {
      // Super admin check, assumption: role serial_no 1 is super admin
      if (state.role && state.role.serial_no == 1) return true;
      if (!state.role || !state.role.permissions) return false;
      return state.role.permissions.some((p) => p.slug === permission_slug);
    },
  },
  actions: {
    ...
    check_is_auth: async function () {
      ...
      if (
        this.is_auth &&
        this.auth_info &&
        Object.keys(this.auth_info).length > 0
      ) {
