<template>
  <li
    ...
    :class="`mm-collapse ${effective_is_show && 'mm-active'}`"
  ...
  >
    ...
    <ul :class="`mm-collapse ${effective_is_show && 'mm-show'}`">
      <template v-for="(menu, index) in visible_menus" :key="index">
        <li
...
export default {
  ...
  computed: {
    ...
    normalized_query() {
      return (this.search_query || "").toLowerCase().trim();
    },
    parent_title_matches() {
      if (!this.normalized_query) return true;
      return (this.menu_title || "").toLowerCase().includes(this.normalized_query);
    },
    visible_menus() {
      // When searching AND parent doesn't match, only show children that match.
      // When parent matches, show all children. No search => show all.
      const q = this.normalized_query;
      if (!q || this.parent_title_matches) return this.menus;
    ...
    matches_search() {
      if (!this.normalized_query) return true;
      return this.parent_title_matches || this.visible_menus.length > 0;
    },
    effective_is_show() {
      // Auto-expand the group while searching so matched children are visible.
      if (this.normalized_query) return true;
      return this.is_show;
    },
  },
