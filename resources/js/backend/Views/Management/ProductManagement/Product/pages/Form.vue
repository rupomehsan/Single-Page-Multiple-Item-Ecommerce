...
export default {
  data: () => ({
    setup,
    form_fields: JSON.parse(JSON.stringify(form_fields)),
    param_id: null,
  }),
  created: async function () {
    let id = (this.param_id = this.$route.params.id);
    await this.load_product_categories();
    this.reset_fields();
    if (id) {
      this.set_fields(id);
    }
  ...
  methods: {
    ...
    load_product_categories: async function () {
      try {
        ...
        const categoryField = this.form_fields.find(f => f.name === 'category_id');
        if (categoryField && data.data?.data) {
          categoryField.data_list = data.data.data.map(cat => ({
