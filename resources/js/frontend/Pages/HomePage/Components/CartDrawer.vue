<template>
  <div class="overlay" :class="{ open: cartOpen }" @click="store.closeCart()"></div>

  <div class="cart-drawer" :class="{ open: cartOpen }">
    <div class="cart-header">
      <h2>🛒 আপনার কার্ট</h2>
      <button class="close-btn" @click="store.closeCart()">✕</button>
    </div>
    ...
    <div class="cart-body">
      ...
      <div v-for="item in cartItems" :key="item.id" class="cart-item">
        ...
        <div class="cart-item-controls">
          <button class="ci-btn" @click="store.removeOne(item.id)">−</button>
          <span class="ci-num">{{ cart[item.id] }}</span>
          <button class="ci-btn" @click="store.addOne(item.id)">+</button>
        </div>
      </div>
    </div>
    ...
    <div class="cart-footer">
      ...
      <button class="checkout-btn" :disabled="cartItems.length === 0 || orderSubmitting" @click="store.drawerCheckout()">
        {{ orderSubmitting ? '⏳ অপেক্ষা করুন…' : '✅ অর্ডার নিশ্চিত করুন' }}
      </button>
      <button class="checkout-btn-wa" :disabled="cartItems.length === 0" @click="store.openWhatsApp()">
...
const { cartOpen, cartItems, cart, cartTotal, orderSubmitting } = storeToRefs(store);
</script>
